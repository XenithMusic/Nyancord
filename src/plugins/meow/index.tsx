...
// Reimplemented using the meow plugin from PawsomeVencord as reference.
// Meow plugin from PawsomeVencord was copied and modified from Equibop
...
const sounds = {
    ...
    "woof": [
        "woof", "bark", "ruff", "arff", "yip", "yap",
        "wuff", "rff", "arf",
        "wooof", "rufff", "arrf", "arrff",
        "woooof", "barkk", "rufff", "arfff", "yipp", "yapp",
        "wufff", "rrff",
        "wuffle", "ruffle", "ruffff", "woofle",
    ],
    "fox": [
        "yip", "yap", "eeeee", "howwl",
        "screech", "bark", "growl", "whine",
        "chatter", "squeal", "scream", "whimper",
        "eeeeee", "howwwl", "screeech",
        "growwwl", "whiiine",
        "squeeal", "screaam", "whimperr",
        "yippp", "yappp", "eee", "howwwwl", "screeeech",
        "barkkk", "growlll", "whiiinee",
    ]
};

async function handleButtonClick() {
    let list = settings.store.soundlist;
    list = sounds[list || "meow"] || sounds.meow;
    const meow = list![Math.floor(Math.random() * list!.length)];
    const channel = getCurrentChannel();
    if (channel) {
        await sendMessage(channel.id, { content: meow });
    }
}

const ChatBarIcon: ChatBarButtonFactory = isMainChat => {
    const showIcon = settings.use(["iconEnabled"]);
    if (!showIcon || !isMainChat) return null;
    return (
...
const settings = definePluginSettings({
    ...
    soundlist: {
        ...
        default: "meow",
        description: "What type of onomotapoeia to use",
        options: [
            { label: "Cat", value: "meow" },
...
export default definePlugin({
    name: "Meow",
    description: "Adds a meow button to the chat bar.",
    authors: [Devs.cookii],
    settings,
